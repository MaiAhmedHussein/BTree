...
public class BTree<K extends Comparable<K>, V> implements IBTree<K, V> {
    ...
    public IBTreeNode<K, V> split(IBTreeNode<K, V> node, IBTreeNode<K, V> child, K key) {
        ...
        left.setKeys(newKeys);
        left.setValues(newValues);
        left.setChildren(newChildren);
        left.setNumOfKeys(newKeys.size());
        ...
        newChildren.add(j, left);
        newChildren.add(j + 1, right);
        ...
        right.setKeys(newKeys);
        right.setValues(newValues);
        right.setChildren(newChildren);
        right.setNumOfKeys(newKeys.size());
